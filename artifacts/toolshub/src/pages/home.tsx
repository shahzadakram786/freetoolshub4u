...
import {
  ...
  FileCode, Braces, Type, Sparkles, Eraser, BookOpen, Clock, ChevronRight, Zap, Lock, Globe
} from "lucide-react";
...
const FEATURES = [
  { icon: Zap, title: "Lightning Fast", desc: "All tools process instantly in your browser. No waiting, no queues.", color: "text-amber-500" },
  { icon: Lock, title: "Privacy First", desc: "Your files and data stay private. We never store your content.", color: "text-emerald-500" },
  { icon: Globe, title: "No Signup Needed", desc: "100% free forever. Use any tool without creating an account.", color: "text-blue-500" },
];
...
export function Home() {
  ...
  return (
    <Layout>
      ...
      <section className="relative w-full py-20 lg:py-32 overflow-hidden flex justify-center border-b border-border/50">
        ...
        <div className="container max-w-7xl mx-auto px-4 relative z-10 text-center">
          ...
          <h1 className="text-5xl lg:text-7xl font-bold mb-6 tracking-tight">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-violet-500">You Will Ever Need.</span>
          </h1>
          <p className="text-lg lg:text-xl text-muted-foreground max-w-2xl mx-auto mb-10">
            AI image tools, live weather, real-time currency rates, plagiarism checker, and more — all free, all instant.
          </p>
          <div className="flex flex-wrap justify-center gap-4">
            <Link href="#tools" className="inline-flex items-center gap-2 bg-primary text-primary-foreground px-8 py-3.5 rounded-xl font-semibold hover:bg-primary/90 transition-colors">
              Browse All Tools <ChevronRight className="w-4 h-4" />
            </Link>
            <Link href="/blog" className="inline-flex items-center gap-2 bg-muted text-foreground px-8 py-3.5 rounded-xl font-semibold hover:bg-muted/80 transition-colors border border-border">
              Read our Blog
            </Link>
          </div>
        </div>
      ...
      <div className="ad-banner w-full max-w-5xl mx-auto px-4 py-4">
        <div className="p-4 rounded-xl bg-muted/30 border text-center text-muted-foreground text-sm">
          [ Google AdSense — Leaderboard ]
        </div>
      </div>

      {/* Features Strip */}
      <section className="w-full py-10 bg-card border-y border-border">
      ...
      <section className="w-full py-16" id="tools">
        <div className="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-10">
            ...
            <p className="text-muted-foreground mt-2">Click any tool to get started instantly. No login, no limits.</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {TOOLS.map((tool) => (
              <Link key={tool.id} href={tool.path}>
                <Card className="h-full p-6 hover:-translate-y-1 hover:shadow-xl hover:border-primary/40 transition-all duration-300 group cursor-pointer flex flex-col items-start text-left relative overflow-hidden">
                  {(tool as any).badge && (
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {blogData.posts.map((post) => (
                <Link key={post.slug} href={`/blog/${post.slug}`}>
                  <Card className="overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group cursor-pointer h-full">
                    ...
                    <div className="p-5">
                      ...
                      <h3 className="font-bold mt-1 mb-2 line-clamp-2 group-hover:text-primary transition-colors">{post.title}</h3>
